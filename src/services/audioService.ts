...
class AudioStreamingService {
  ...
  private handleAudioChunk(socket: Socket, audioData: Buffer): void {
    ...
    if (!session) {
      logger.warn('Audio chunk received for inactive session', { socketId: socket.id });
      return;
    }
    ...
    try {
      ...
      const stream = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are Jarvis, a helpful AI assistant. Provide concise, natural responses suitable for voice conversation.'
          },
          {
            role: 'user',
            content: userMessage
