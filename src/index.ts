...
function bindAuthUser(req: AuthedRequest, res: express.Response, next: express.NextFunction) {
  const headerUser = (req.headers['x-user-id'] as string) || (req.headers['x-user'] as string);
...
import { MediaSafetyService, MediaSafetyDecision } from './services/mediaSafetyService';
...
const visionAgent = new VisionAgent(prisma, cameraService, rtspStreamService, cvService, recordingService);
...
function allowUnlessBlocked(decision: MediaSafetyDecision): MediaSafetyDecision {
  if (decision.action === 'block') {
    return decision;
  }
  if (decision.action === 'sanitize') {
    return { ...decision, action: 'allow' as const, reason: `${decision.reason} (coerced allow)` };
  }
  return decision;
}
...
app.post('/api/music/generate', requireMediaEnabled, async (req: express.Request, res: express.Response) => {
  try {
    ...
    if (decision.action === 'block') {
      return res.status(403).json({ error: 'Blocked by media safety', decision });
    }
...
// Media (images)
app.post('/api/media/image/:action', requireMediaEnabled, async (req: express.Request, res: express.Response) => {
  try {
    const action = req.params.action as
      ...
      | 'outpaint'
      | 'upscale';
    const { prompt, style, seed, imageUrl, maskUrl, strength, width, height, userId } = req.body;

    if (action === 'generate' && !prompt) {
      return res.status(400).json({ error: 'Missing prompt' });
    }
    if (action !== 'generate' && !imageUrl) {
      return res.status(400).json({ error: 'Missing imageUrl for edit action' });
    }
    ...
    let result;
    if (action === 'generate') {
      result = await imageService.generate({ prompt, style, seed, width, height });
    } else if (action === 'variation') {
      result = await imageService.variation({ imageUrl, prompt, style, seed, strength });
    } else if (action === 'inpaint') {
      result = await imageService.inpaint({ imageUrl, maskUrl, prompt, style, strength });
    } else if (action === 'outpaint') {
      result = await imageService.outpaint({ imageUrl, prompt, style, strength });
    } else if (action === 'upscale') {
      result = await imageService.upscale({ imageUrl, prompt, style, strength });
    } else {
    ...
    res.json({ success: true, asset: assetStorage.resolveDelivery(stored), decision });
  } catch (error) {
...
app.post('/api/media/video/generate', requireMediaEnabled, async (req: express.Request, res: express.Response) => {
  try {
    ...
    const job = await videoService.generate({ prompt, style, durationSeconds });
    ...
    res.json({ success: true, job: assetStorage.resolveDelivery(stored), decision });
  } catch (error) {
...
app.post('/api/media/video/edit', requireMediaEnabled, async (req: express.Request, res: express.Response) => {
  try {
    const { action, sources, startSeconds, endSeconds, overlayUrl, prompt, style, userId } = req.body;
...
app.get('/api/media/video/status/:id', requireMediaEnabled, async (req: express.Request, res: express.Response) => {
  try {
...
app.get('/api/media/video/list', requireMediaEnabled, (_req: express.Request, res: express.Response) => {
  const assets = assetStorage.list().filter((a) => a.type === 'video');
  res.json({ videos: assets });
});

app.get('/api/media/assets', requireMediaEnabled, (_req: express.Request, res: express.Response) => {
  const assets = assetStorage.list();
  res.json({ assets });
});

app.get('/api/media/admin/summary', requireMediaEnabled, (_req: express.Request, res: express.Response) => {
  const assets = assetStorage.list(50);
  const safety = mediaSafety.getRecent(50);
  res.json({ assets, safety });
});

app.get('/api/music/status/:id', requireMediaEnabled, async (req: express.Request, res: express.Response) => {
  try {
    const id = req.params.id;
...
app.get('/api/music/list', requireMediaEnabled, (_req: express.Request, res: express.Response) => {
...
app.post(
  ...
  async (req: AuthedRequest, res: express.Response) => {
    try {
      ...
      const response = await dialogueAgent.generateResponse(input, sid, userId, conversationId);
      const resolvedConversationId = conversationId || dialogueAgent['sessionToConversation']?.get?.(sid) || null;
      res.json({ success: true, response, sessionId: sid, conversationId: resolvedConversationId });
    } catch (error) {
...
app.get('/api/dialogue/conversations/:userId', bindAuthUser, async (req: AuthedRequest, res: express.Response) => {
  try {
...
if (ORCHESTRATOR_ENABLED && orchestrator) {
  app.post('/api/orchestrator/message', async (req: express.Request, res: express.Response) => {
    try {
...
app.get('/api/voice/status/:userId', requireTenantToken, async (req: express.Request, res: express.Response) => {
  try {
    const { userId } = req.params;
...
app.get('/api/spotify/callback', async (req: express.Request, res: express.Response) => {
  try {
    ...
    return res.send('Spotify authorization successful. You can close this window.');
  } catch (error) {
...
app.post('/api/spotify/refresh', async (req: express.Request, res: express.Response) => {
  try {
...
app.post('/api/media/safety/evaluate', requireMediaEnabled, (req: express.Request, res: express.Response) => {
  try {
    const decision = mediaSafety.evaluate({
...
app.get('/api/media/safety/events', requireMediaEnabled, (_req: express.Request, res: express.Response) => {
...
app.get('/api/truelayer/auth', (_req: express.Request, res: express.Response) => {
...
app.get('/api/truelayer/callback', async (req: express.Request, res: express.Response) => {
  try {
    const code = req.query.code as string;
    const state = req.query.state as string;
    if (!code || !state) {
    ...
    const token = await truelayerClient.exchangeCode(code, entry.codeVerifier);
    ...
    return res.send('TrueLayer authorization successful. You can close this window.');
  } catch (error) {
...
async function ensureFreshTrueLayerToken(
  ...
  if (connection.expiresAt.getTime() - Date.now() < 60_000) {
    const refreshed = await truelayerClient.refreshToken(connection.refreshToken);
...
app.post('/api/truelayer/sync', async (req: express.Request, res: express.Response) => {
  try {
    ...
    const { connection, accessToken } = await ensureFreshTrueLayerToken(bankConnectionId);
    await financeService.syncTrueLayer(connection.userId, connection, accessToken, { from, to });
...
app.post('/api/truelayer/payments', async (req: express.Request, res: express.Response) => {
  try {
    const { bankConnectionId, amount, currency, reference, beneficiary } = req.body;
...
app.get('/api/truelayer/payments/:id', async (req: express.Request, res: express.Response) => {
  try {
    ...
    const { accessToken } = await ensureFreshTrueLayerToken(payment.bankConnectionId);
    const updated = await paymentService.updatePaymentStatus(paymentId, accessToken);
...
function requireSystemEnabled(req: express.Request, res: express.Response, next: express.NextFunction) {
  if (!SYSTEM_ENABLED) {
...
app.post('/api/system/exec', requireSystemEnabled, async (req: express.Request, res: express.Response) => {
  try {
...
app.post('/api/system/fix/:action', requireSystemEnabled, async (req: express.Request, res: express.Response) => {
  try {
    const action = req.params.action as Parameters<RemediationLibrary['run']>[0];
...
app.post('/api/system/registry', requireSystemEnabled, async (req: express.Request, res: express.Response) => {
  try {
    const { path: regPath, name, value, mode } = req.body;
    if (!regPath || !mode) {
      return res.status(400).json({ error: 'Missing registry path or mode' });
    }
    const result = await systemControlAgent.registry({ path: regPath, name, value, mode });
...
app.post('/api/system/service/restart', requireSystemEnabled, async (req: express.Request, res: express.Response) => {
  try {
    const { name } = req.body;
    if (!name) {
...
app.get('/api/system/processes', requireSystemEnabled, async (_req: express.Request, res: express.Response) => {
  try {
...
if (process.env.VISION_ENABLED !== 'false') {
  // Camera Management
  app.post('/api/vision/cameras', async (req: express.Request, res: express.Response) => {
    try {
      const { name, protocol, ipAddress, rtspUrl, username, password, model } = req.body;
      ...
      const id = await visionAgent.addCamera({ name, protocol, ipAddress, rtspUrl, username, password, model });
  ...
  app.get('/api/vision/cameras', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.get('/api/vision/cameras/:id', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.put('/api/vision/cameras/:id', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.delete('/api/vision/cameras/:id', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.post('/api/vision/cameras/:id/connect', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.post('/api/vision/cameras/:id/disconnect', async (req: express.Request, res: express.Response) => {
    try {
      await visionAgent.disconnectCamera(req.params.id);
      res.json({ success: true });
    } catch (error) {
  ...
  app.post('/api/vision/cameras/discover', async (req: express.Request, res: express.Response) => {
    try {
  ...
  // Streaming
  app.post('/api/vision/streams/:cameraId/start', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.post('/api/vision/streams/:cameraId/stop', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.get('/api/vision/streams/:cameraId', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.get('/api/vision/streams', async (_req: express.Request, res: express.Response) => {
    try {
  ...
  // PTZ Control
  app.post('/api/vision/cameras/:id/ptz/move', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.post('/api/vision/cameras/:id/ptz/stop', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.get('/api/vision/cameras/:id/ptz/status', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.post('/api/vision/cameras/:id/ptz/preset', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.get('/api/vision/cameras/:id/ptz/presets', async (req: express.Request, res: express.Response) => {
    try {
  ...
  // Object Detection
  app.post('/api/vision/detect/:cameraId', async (req: express.Request, res: express.Response) => {
    try {
      const { frameData, frameUrl, confidenceThreshold } = req.body;
      const detections = await visionAgent.detectObjects(req.params.cameraId, {
  ...
  app.get('/api/vision/detections', async (req: express.Request, res: express.Response) => {
    try {
      const filters: any = {};
      if (req.query.cameraId) filters.cameraId = req.query.cameraId as string;
      if (req.query.objectType) filters.objectType = req.query.objectType as string;
  ...
  app.get('/api/vision/detections/:id', async (req: express.Request, res: express.Response) => {
    try {
  ...
  // Recording
  app.post('/api/vision/recordings/start', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.post('/api/vision/recordings/:id/stop', async (req: express.Request, res: express.Response) => {
    try {
  ...
  app.get('/api/vision/recordings', async (req: express.Request, res: express.Response) => {
    try {
      const filters: any = {};
      if (req.query.cameraId) filters.cameraId = req.query.cameraId as string;
  ...
  app.get('/api/vision/recordings/:id/playback', async (req: express.Request, res: express.Response) => {
    try {
      const url = await visionAgent.getPlaybackUrl(req.params.id);
      if (!url) return res.status(404).json({ error: 'Recording not found or not available' });
