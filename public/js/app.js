...
class JarvisApp {
    ...
    setupSocketIO() {
        ...
        this.socket.on('error', (error) => {
            ...
            this.addMessage('system', `Error: ${error.message || 'An error occurred'}`);
    ...
    checkBrowserSupport() {
        ...
        if (!this.recognition) {
            issues.push('Web Speech API not available (voice input will use server transcription)');
        }
    ...
    async sendMessage() {
        ...
        setTimeout(() => {
            this.addMessage('assistant', 'I received your message. Voice conversation is available via the microphone button.');
        }, 500);
    }
